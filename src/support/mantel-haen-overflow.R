    ...
    "less", "greater"), correct = TRUE, exact = FALSE, conf.level = 0.95) 
{
    ...
    if ((I == 2) && (J == 2)) {
        ...
        if (!exact) {
            ...
            sd <- sqrt(sum((as.double(x[1, 1, ]) + as.double(x[2, 2, ])) * as.double(x[1, 
                1, ] * x[2, 2, ]/n^2))/(2 * s.diag^2) + sum(((as.double(x[1, 
                1, ]) + as.double(x[2, 2, ])) * as.double(x[1, 2, ]) * as.double(x[2, 1, 
                ]) + (as.double(x[1, 2, ]) + as.double(x[2, 1, ])) * as.double(x[1, 1, ]) * 
                as.double(x[2, 2, ]))/n^2)/(2 * s.diag * s.offd) + sum((as.double(x[1, 
                2, ]) + as.double(x[2, 1, ])) * as.double(x[1, 2, ]) * as.double(x[2, 1, 
                ])/n^2)/(2 * s.offd^2))
        ...
        for (k in 1:K) {
            ...
            V <- V + (kronecker(diag(ntot * colsums, nrow = J - 
                ...
                nrow = I - 1L) - outer(rowsums, rowsums))/(as.double(ntot)^2 * 
                (as.double(ntot) - 1)))
        }
        n <- n - m
