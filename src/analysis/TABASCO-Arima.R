...
ggplot() +
  ...
  # Prediction;
  geom_line(aes(x = test_set$Date[1:58], y = predict.fit$lower), col = "gray", linetype = "dashed") +
  geom_line(aes(x = test_set$Date[1:58], y = predict.fit$upper), col = "gray", linetype = "dashed") +
  geom_line(aes(x = test_set$Date[1:58], y = predict.fit$mean), col = "red2") +
  
  # Confidence;
  geom_ribbon(aes(x = test_set$Date[1:58], ymin = predict.fit$lower, ymax = predict.fit$upper), fill = "gray", alpha = 0.2) +
