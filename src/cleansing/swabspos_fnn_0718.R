...
swabspos[, -2] <- apply(X = swabspos[, -2], MARGIN = 2, FUN = function(x){ifelse(as.character(x) == "No", 0, 1)});

write.csv(swabspos, file = gzfile("~/TABASCO-MEXCOV-19/data/cleansed/0718/swabspos_fnn_0718.csv.gz"), row.names = FALSE);
